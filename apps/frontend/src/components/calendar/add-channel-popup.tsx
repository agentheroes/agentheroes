...
export function AddChannelPopup({ isOpen, onClose }: AddChannelPopupProps) {
  const [availableSocials, setAvailableSocials] = useState<AvailableSocialMedia[]>([]);
  ...
  const fetchAvailableSocials = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch available social media: ${response.status}`);
      }
      ...
      setError('Failed to load available social media. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleConnectSocial = async (identifier: string) => {
    try {
      ...
      const response = await fetch(`/socials/url?identifier=${identifier}&referer=${encodeURIComponent(referer)}&timezone=${offset}`);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-900 rounded-lg shadow-lg max-w-md w-full max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center border-b border-gray-800 p-4">
          <h2 className="text-lg font-semibold text-white">Connect a Channel</h2>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {isLoading && (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-2 text-gray-400">Loading available channels...</span>
            </div>
          )}
...
function SocialMediaCard({ identifier, onClick }: SocialMediaCardProps) {
  ...
  const getDisplayName = (identifier: string): string => {
    ...
    return socialNames[identifier] || identifier.charAt(0).toUpperCase() + identifier.slice(1);
  };
    ...
    >
      <div className="w-12 h-12 rounded-full overflow-hidden flex-shrink-0 bg-gray-700">
        <img
          ...
          alt={`${getDisplayName(identifier)} logo`}
      ...
      <div className="ml-4 text-left">
        <div className="font-medium text-white">{getDisplayName(identifier)}</div>
