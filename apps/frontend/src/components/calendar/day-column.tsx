...
export function DayColumn({ day, timeSlots, events, isToday }: DayColumnProps) {
  ...
  return (
    <div className={`flex-1 border-r border-gray-800 min-w-[100px] ${isToday ? 'bg-gray-900' : ''}`}>
      {/* Day header */}
      <div className={`h-14 flex flex-col items-center justify-center border-b border-gray-800 ${isToday ? 'bg-gray-800' : ''}`}>
        <span className="text-xs font-medium text-gray-400">{day.name.toUpperCase()}</span>
        <span className={`text-xl font-bold ${isToday ? 'text-white' : 'text-gray-300'}`}>{day.date.getDate()}</span>
      </div>
